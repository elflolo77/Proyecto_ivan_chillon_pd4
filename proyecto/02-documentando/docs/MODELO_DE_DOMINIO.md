...
3. **`Sesion`**: Engranaje puente entre el Tiempo y las entidades Físicas (`Películas + Salas`).
   - El estado de la sesión (`_estado` == `"programada" | "completa" | "cancelada"`) y el sumatorio incremental (`_asientos_ocupados`) restringen de manera estricta el acceso masivo a su propiedad derivada: `capacidad_disponible()`.
